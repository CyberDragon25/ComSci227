...
public class FourOfAKindEvaluator extends AbstractEvaluator
{
  ...
  public boolean canSatisfy(Card[] mainCards) {
    ...
    if (mainCards.length == cardsRequired()){
      if(mainCards[0].getRank() == mainCards[1].getRank()
          && mainCards[0].getRank() == mainCards[2].getRank()
          && mainCards[0].getRank() == mainCards[3].getRank()
              ){
        satisty = true;
      }
    }
  ...
  private ArrayList<int[]> getValidSubsets(Card[] allCards){
    ...
    ArrayList<int[]> subsets =  SubsetFinder.findSubsets(allCards.length, handSize());
    for(int i = 0; i < subsets.size(); i++){
      Card[] subsetCards = cardsMatchingSubsets(allCards, subsets.get(i));
      if(subsetCards[0].getRank() == subsetCards[1].getRank() &&
              subsetCards[0].getRank() == subsetCards[2].getRank() &&
              subsetCards[0].getRank() == subsetCards[3].getRank() && // this for suites after this
              subsetCards[0].getSuit() != subsetCards[1].getSuit() &&
              subsetCards[0].getSuit() != subsetCards[2].getSuit() &&
              subsetCards[0].getSuit() != subsetCards[3].getSuit()
      ){
